...
# =========================
# App (pygbag-compatible loop edge)
# =========================

def app_dir() -> Path:
    # In-browser (Emscripten) packaging usually expects relative paths.
...
def p(path: Path) -> str:
    # Normalize to forward slashes (web-friendly).
...
async def main() -> None:
    ...
    # Optional pygbag nicety (only exists in pygbag’s injected platform module)
    win_mod = getattr(platform, "window", None)
    if win_mod is not None and hasattr(win_mod, "window_resize"):
        win_mod.window_resize()

    rl.set_target_fps(60)
    bg_color = hex_to_color(BG)

    # [HOW] load textures/fonts once (GPU + font decode)
    card_front = rl.load_texture(p(Path(cfg.asset("resources", "images", "card_front.png"))))
    card_back = rl.load_texture(p(resources / "images" / "card_back.png"))
    wrong_tex = rl.load_texture(p(resources / "images" / "wrong.png"))
    right_tex = rl.load_texture(p(resources / "images" / "right.png"))

    title_font = rl.load_font_ex(
        p(resources / "fonts" / "Roboto-Italic.ttf"),
        theme.title_font_size,
        None,
        0,
    )
    word_font = rl.load_font_ex(
        p(resources / "fonts" / "Roboto-Bold.ttf"),
        theme.word_font_size,
        None,
        0,
    )
    ...
    try:
        ...
        while not rl.window_should_close():
            ...
            # [WHEN -> QUEUES] schedule auto-flip intent
            if (not model.showing_back) and (time_since_next >= policy.flip_delay_s):
                events.append(Event.AUTO_FLIP)

            # [HOW -> QUEUES] input edge emits intents
            if rl.is_mouse_button_pressed(rl.MOUSE_BUTTON_LEFT):
                mp = rl.get_mouse_position()
                if rl.check_collision_point_rec(mp, wrong_rect) or rl.check_collision_point_rec(mp, right_rect):
            ...
            while events:
                ev = events.popleft()
                if ev is Event.NEXT:
                    chosen = random.choice(word_pairs)  # [HOW]
                    model = next_card(model, chosen)    # [WHAT]
                    time_since_next = 0.0               # [WHEN]
            ...
            # REQUIRED by pygbag: yield to the browser event loop
