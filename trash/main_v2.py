...
class ThemeSpec:
    ...
    def key(self) -> Tuple:
        ...
        return (
            ...
            self.title_font_path, self.word_font_path,
            self.title_font_size, self.word_font_size,
            self.canvas_size, self.title_pos, self.word_pos,
            self.title_color, self.word_color
        )
...
class RandomSelection:
    ...
    def next(self, current: WordPair, corpus: tuple[WordPair, ...]) -> WordPair:
        if not corpus: return current
        nxt = random.choice(corpus)
        return nxt if nxt != current else random.choice(corpus)

@dataclass(frozen=True)
class FlipPolicy:
    # Placeholder for richer rules (e.g., spaced repetition)
...
class CardRenderer:
    ...
    def render(self, words: WordPair) -> CardImages:
        ...
        df = ImageDraw.Draw(f)
        df.text(self.theme.spec.title_pos, "French", anchor="mm",
                font=self.theme.title_font, fill=self.theme.spec.title_color)
        df.text(self.theme.spec.word_pos,  words.fr, anchor="mm",
                font=self.theme.word_font,  fill=self.theme.spec.word_color)

        db = ImageDraw.Draw(b)
        db.text(self.theme.spec.title_pos, "English", anchor="mm",
                font=self.theme.title_font, fill=self.theme.spec.title_color)
        db.text(self.theme.spec.word_pos,  words.en, anchor="mm",
                font=self.theme.word_font,  fill=self.theme.spec.word_color)
...
class TogaViewPort(ViewPort):
    """Wraps the Toga widget tree; no domain leakage."""
    def __init__(self, body_row: toga.Row, width: int = TARGET_W, height: int = TARGET_H):
        self.body = body_row
        self.width = width
        self.height = height
    ...
    def show(self, front: Image.Image, back: Image.Image, show_back: bool) -> None:
        self.front_view = toga.ImageView(front, style=Pack(width=self.width, height=self.height))
        self.back_view  = toga.ImageView(back,  style=Pack(width=self.width, height=self.height))
        self._showing_back = show_back
        view = self.back_view if show_back else self.front_view
        if self.body.children:
            self.body.replace(self.body.children[0], view)
        else:
            self.body.add(view)
    ...
    def update_side(self, show_back: bool) -> None:
        ...
        if self.body.children:
            self.body.replace(self.body.children[0], new_view)
        else:
            self.body.add(new_view)

class IntervalScheduler(Scheduler):
    """Timing policy lives here, not in the app/controller."""
    def __init__(self, interval_sec: float, event_name: str = "flip", enabled: bool = True):
...
class AsyncEventBus:
    ...
    async def publish(self, evt: str) -> None:
...
class Controller:
    ...
    # event handler
    def _on_event(self, evt: str) -> None:
        if evt == "flip" and self.flip_policy.auto_flip:
            self.flip()
        elif evt == "next":
            self.next()
...
def load_theme(spec: ThemeSpec) -> Theme:
    ...
    return Theme(spec=spec, base_front=base_front, base_back=base_back,
                 title_font=title_font, word_font=word_font)
...
class Milo(toga.App):
    def startup(self) -> None:
        ...
        wrapper = toga.Column(style=Pack(align_items=CENTER, justify_content=CENTER, padding=10))
        page    = toga.Column(style=Pack(flex=1, align_items=CENTER, justify_content=CENTER, padding=10))
        ...
        viewport = TogaViewPort(body)

        # --- event bus + scheduler injection ---
        self.bus = AsyncEventBus()
        self.flip_sched = IntervalScheduler(interval_sec=5.0, event_name="flip", enabled=True)
        ...
        # --- buttons (emit intents, not mutate UI) ---
        next_btn = toga.Button("Next", on_press=lambda b: asyncio.create_task(self.bus.publish("next")))
        flip_btn = toga.Button("Flip", on_press=lambda b: asyncio.create_task(self.bus.publish("flip")))
